...
tables_clear_script_filename="psql-tables-clear-script.sql"

if [ -f "$tables_clear_script_filename" ]; then
    ...
    if [ -n "${pg_user}" ]; then
      ...
      if [ -n "${pg_host}" ]; then
        ...
        if [ -n "${pg_port}" ]; then
          ...
          psql -U "$pg_user" -h "$pg_host" -p "$pg_port" -c '\c task_planner' -f "$tables_clear_script_filename" -e
  ...
  echo "File tables_clear_script_filename not found!"
fi
